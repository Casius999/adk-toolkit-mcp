...
**adk-toolkit-mcp** MCP server (94 tools across 17 domains — including the **Workflow** graph
engine, the **Skill Registry**, and **planners**; SOTA as of June 2026). The server is a
**code-first sidecar**: you author agents by calling MCP tools that maintain a sidecar
(`.adk_toolkit/agents.json` + `runtime.json`) and **regenerate `agent.py` wholesale** from it.
Never hand-edit generated `agent.py` — call a tool and it is re-rendered. Generated code is held to
...
"I want to do X → call this exact tool" bridge covering all 94 tools — open it whenever unsure.**
...
| Choose an agent type; sub_agents vs AgentTool vs RemoteA2aAgent; Loop/Parallel; set root; attach a planner (BuiltInPlanner/PlanReActPlanner) | `references/01-agent-types.md` | `agents_*` |
...
| Orchestrate a graph: Workflow nodes (agent/function/join) + edges (conditional/cyclic/ReAct); set entry/root | `references/01-agent-types.md` (Workflow section) | `workflow_*` |
| Author + attach Agent Skills (SKILL.md skills, SkillToolset, registry inventory) | `references/02-tools.md` (Skills section) | `skills_*` |
...
| **Complete task → exact MCP tool(s) map (all 94 tools, by domain)** | `references/13-tool-catalog.md` | **all** |

## The 17 domains at a glance

`project` (scaffold/inspect) · `agents` (compose + planners) · `tools` (attach tools) ·
`models` (model/config) · `sessions` · `memory` · `artifacts` (runtime state) ·
`workflow` (graph orchestration) · `skills` (Agent Skill Registry) · `run` (execute) ·
`eval` (evaluate) · `deploy` · `dev` (CLI servers) · `a2a` · `mcp_bridge` (interop) ·
`safety` · `observability` (ops).

These are the 17 tool-exposing sub-servers; `project_model` and `runtime` are internal support
modules (not exposed as domains). Exposed tool names are always `<domain>_<bare>` (e.g.
`agents_create_llm`, `agents_set_planner`, `tools_add_mcp_toolset`, `workflow_add_node`,
`skills_attach`, `run_agent`, `eval_run`, `deploy_cloud_run`). The MCP server is `adk-toolkit`
(run via `uv run adk-toolkit-mcp`). Two resources: `adk://version`, `adk://models`.
...
2. `agents_create_llm / create_sequential / …` then `agents_set_root` — build the agent graph.
   (For non-linear / conditional / cyclic orchestration, build a `workflow_*` graph instead and
   `workflow_set_root` — the modern successor to the deprecated Sequential/Parallel/Loop agents.)
3. `tools_add_*` — attach tools to LlmAgents (only LlmAgents carry tools).
4. `models_set / configure_litellm / generate_config` — pick model + sampling/safety. Optionally
   `agents_set_planner` to make an LlmAgent plan before acting (BuiltInPlanner / PlanReActPlanner).
...
Add `safety_*` (guardrails), `skills_*` (author/attach Agent Skills via SkillToolset),
`a2a_*` (expose/consume), `observability_*` (tracing) as the task needs. When in doubt about which
tool implements a step, consult `references/13-tool-catalog.md`.
